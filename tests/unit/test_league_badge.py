...
def test_contract_is_paused_after_minting():
    # Arrange
    ...
    # Act
    tx_award = league_badge.mintLeagueBadge(badge_awardee, {"from": account})
    tx_award.wait(1)
...
def test_contract_is_paused_after_burning():
    # Arrange
    if network.show_active() not in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        pytest.skip()
    account = get_account()
    ...
    tx_award.wait(1)
    state_after_minting = league_badge.paused()
    tx_burn = league_badge.burnLeagueBadge(tx_award.return_value, {"from": account})
    tx_burn.wait(1)
...
def test_badge_id_is_incremented_correctly():
    ...
    # Act
    tx_award_1 = league_badge.mintLeagueBadge(badge_awardee_1, {"from": account})
    tx_award_1.wait(1)
    tx_award_2 = league_badge.mintLeagueBadge(badge_awardee_2, {"from": account})
    tx_award_2.wait(1)
    # Assert
...
def test_badge_is_minted_successfully(URI):
    ...
    tx_award = league_badge.mintLeagueBadge(badge_awardee, {"from": account})
    tx_award.wait(1)
    tx_set_token_uri = league_badge._setTokenURI(
        tx_award.return_value, URI, {"from": account}
    )
...
def test_member_can_have_just_one_badge(URI):
    ...
    # Assert
    with pytest.raises(exceptions.VirtualMachineError):
        league_badge.mintLeagueBadge(badge_awardee, {"from": account})
...
def test_badge_cannot_be_burned_by_owner(URI):
    ...
    # Assert
    with pytest.raises(exceptions.VirtualMachineError):
        tx_transfer = league_badge.burnLeagueBadge(
...
def test_badge_cannot_be_burned_twice(URI):
    ...
    tx_set_token_uri.wait(1)
    tx_burn_1 = league_badge.burnLeagueBadge(tx_award.return_value, {"from": account})
    tx_burn_1.wait(1)
    # Assert
    with pytest.raises(exceptions.VirtualMachineError):
        league_badge.burnLeagueBadge(tx_award.return_value, {"from": account})
